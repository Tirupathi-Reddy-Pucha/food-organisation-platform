...
function App() {
    ...
    const [scanTargetId, setScanTargetId] = useState(null); // The item ID we are trying to scan for
    ...
    // [NEW] Google Translate Script Injection
    useEffect(() => {
            ...
            window.googleTranslateElementInit = () => {
                if (window.google && window.google.translate) {
                    new window.google.translate.TranslateElement(
                        {
                            pageLanguage: 'en',
                            includedLanguages: 'en,hi,bn,te,ta,mr,fr,es', // Restrict languages
    ...
    const isExpired = (createdAt, expiryHours) => {
        const created = new Date(createdAt).getTime();
        const expires = created + (expiryHours * 60 * 60 * 1000);
    ...
    const fetchUserData = useCallback(async () => {
        ...
        try {
            ...
            if (userData._id && !userData.id) userData.id = userData._id; // Normalize ID
            setUser(userData);
            localStorage.setItem('user_credits', userData.credits || 0);
    ...
    useEffect(() => {
        ...
        fetchUserData();
    }, [fetchListings, fetchStats, fetchUserData, leaderboardTab]);
            ...
            try {
                ...
                scanner.render(async (decodedText) => {
                    ...
                    if (decodedText === `FOOD_ID:${scanTargetId}`) {
                        ...
                        // Directly call update logic here
                        try {
                            const token = localStorage.getItem('token');
                            await axios.put(`${API_URL}/listings/${scanTargetId}/status`,
                                { status: 'Delivered' },
                                { headers: { 'x-auth-token': token } }
                            );
                            showToast("✅ Delivery Verified via QR!");
                            fetchListings();
                            fetchMyHistory();
                            fetchStats();
                        } catch (err) {
                        ...
                        alert("❌ Invalid QR Code! Please scan the correct Donor QR.");
                    }
                }, (error) => {
    ...
    const updateStatus = async (id, newStatus, proofUrl = null) => {
        ...
        if (newStatus === 'Cancelled') {
            ...
            if (!reason) return;
        } else if (newStatus !== 'ReadyToPickup') {
            if (!window.confirm(`Mark as ${newStatus}?`)) return;
        }

        // [NEW] Volunteer must provide pickup proof
        if (newStatus === 'In Transit' && user.role === 'Volunteer' && !proofUrl) {
            return showToast("⚠️ Please upload a photo of the QR code first!", "error");
        }

        // [NEW FROM CODE 2] Check for delivery proof (if any other role needs it)
        ...
        try {
            await axios.put(`${API_URL}/listings/${id}/status`,
                { status: newStatus, reason: reason, pickupProof: proofUrl || deliveryProof },
        ...
        <div className={`App ${isRTL ? 'rtl-mode' : ''} ${fontSize}`} style={{ ...currentBodyStyle, padding: '30px 20px', maxWidth: '850px', margin: '0 auto' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '10px' }}>
                ...
                <div style={{ display: 'flex', gap: '10px', alignItems: 'center', flexWrap: 'wrap' }}>
                    {user.id && (
                        <div className="header-user" style={{ display: 'flex', alignItems: 'center', gap: '10px', background: highContrast ? '#222' : 'rgba(255,255,255,0.8)', padding: '5px 15px', borderRadius: '30px', border: highContrast ? '1px solid #ffd700' : 'none' }}>
                            <UserAvatar name={user.name} />
                            <span onClick={() => setView('profile')} style={{ cursor: 'pointer', fontWeight: 'bold', display: 'flex', alignItems: 'center', gap: '5px' }}>
                                {user.name} {user.isVerified && '☑️'}
                            </span>
                            <button onClick={logout} style={{ ...buttonStyle('#e74c3c'), width: 'auto', padding: '5px 10px', fontSize: '0.8em', marginTop: 0, marginLeft: '10px' }}>{t.logout}</button>
                        </div>
                    )}
                    <button onClick={cycleFontSize} title="Change Text Size" style={{ background: '#3498db', color: 'white', border: 'none', padding: '5px 10px', borderRadius: '5px', cursor: 'pointer', fontWeight: 'bold', fontSize: '0.9em', height: '35px' }}>
                        A+
                    </button>
                    <div id="google_translate_element" style={{ display: 'inline-block', height: '35px', overflow: 'hidden' }}></div>
                    <button onClick={() => setHighContrast(!highContrast)} style={{ background: highContrast ? '#fff' : '#2c3e50', color: highContrast ? '#000' : '#fff', border: 'none', padding: '5px 10px', borderRadius: '5px', cursor: 'pointer', fontWeight: 'bold', fontSize: '0.9em', height: '35px' }}>
            ...
            {/* AUTH (ONLY SHOWN IF NOT LOGGED IN AND SESSION CHECKED) */}
            {!user.id && isSessionChecked && (
                <div className="card" style={currentCardStyle}>
            ...
            {/* PROFILE VIEW (ONLY IF LOGGED IN AND VIEWING PROFILE) */}
            {user.id && view === 'profile' && (
                <div className="card" style={currentCardStyle}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
                        ...
                        <button onClick={() => setView('feed')} style={{ ...buttonStyle('#95a5a6'), width: 'auto', padding: '8px 20px', fontSize: '0.9em', marginTop: 0 }}>Back to Feed 🏠</button>
                    </div>

                    <div style={{ background: highContrast ? '#222' : '#f8f9fa', padding: '20px', borderRadius: '12px', border: highContrast ? '1px solid #ffd700' : 'none' }}>
                        {!isEditingProfile ? (
                            <div>
                                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                    <h4>👤 Account Details</h4>
                                    <button onClick={startEditing} style={{ ...buttonStyle(highContrast ? '#444' : '#3498db'), width: 'auto', padding: '5px 15px', fontSize: '0.8em' }}>✏️ Edit Profile</button>
                                </div>
                                <p style={{ margin: '10px 0' }}><strong>Email:</strong> {user.email}</p>
                                <p style={{ margin: '10px 0' }}><strong>Phone:</strong> {user.phone}</p>
                                <p style={{ margin: '10px 0' }}><strong>Address:</strong> {user.address}</p>

                                <div style={{ margin: '20px 0', padding: '20px', background: 'linear-gradient(to right, #6dd5fa, #2980b9)', color: 'white', borderRadius: '12px', fontWeight: 'bold', textAlign: 'center' }}>
                                    <div style={{ fontSize: '1.2em' }}>🌱 Sustainability Credits</div>
                                    <div style={{ fontSize: '2.5em' }}>{user.credits || 0}</div>
                                </div>

                                {calculateMyRating() < 3 && <div style={{ background: '#ffcccc', color: 'red', padding: '10px', borderRadius: '8px', fontSize: '0.9em', marginTop: '5px', fontWeight: 'bold' }}>⚠️ Warning: Low Impact Rating!</div>}

                                <div style={{ marginTop: '20px' }}>
                                    <strong>Status:</strong> {user.isVerified ? <span style={{ color: 'green', marginLeft: '5px', fontWeight: 'bold' }}>✅ Verified Member</span> : <span style={{ color: 'orange', marginLeft: '5px' }}>⚠️ Not Verified</span>}
                                </div>

                                {user.role === 'NGO' && (
                                    <div style={{ background: '#e3f2fd', padding: '15px', marginTop: '15px', borderRadius: '8px' }}>
                                        <strong>🏢 NGO Operations:</strong> <br />
                                        Fridge: {user.ngoCapacity?.fridge || 'N/A'}, Dry Store: {user.ngoCapacity?.dryStorage || 'N/A'} <br />
                                        Groups: {user.servedGroups || 'General'}
                                    </div>
                                )}

                                {user.role === 'Volunteer' && (
                                    <div style={{ marginTop: '15px', padding: '15px', background: volunteerAvailable ? '#d4edda' : '#f8d7da', borderRadius: '8px' }}>
                                        <label style={{ cursor: 'pointer', display: 'flex', alignItems: 'center', fontWeight: 'bold' }}>
                                            <input
                                                type="checkbox"
                                            ...
                                            />
                                            {volunteerAvailable ? "✅ Ready for Pickups" : "❌ On Break"}
                                        </label>
                                    </div>
                                )}

                                {user.role === 'Donor' && (
                                    <div style={{ marginTop: '20px', padding: '15px', background: highContrast ? '#000' : 'white', border: highContrast ? '1px solid #ffd700' : '1px solid #ddd', borderRadius: '12px', display: 'flex', justifyContent: 'space-around', alignItems: 'center' }}>
                                        <div style={{ textAlign: 'center' }}><span style={{ fontSize: '0.8em', color: '#777' }}>IMPACT RANK</span><div style={{ fontWeight: 'bold', fontSize: '1.2em', color: '#e67e22' }}>{getDonorBadge(myListings.length)}</div></div>
                                        <div style={{ textAlign: 'center' }}><span style={{ fontSize: '0.8em', color: '#777' }}>RATING</span><div style={{ fontWeight: 'bold', fontSize: '1.2em', color: '#f1c40f' }}>{calculateMyRating()} ★</div></div>
                                    </div>
                                )}

                                <div style={{ marginTop: '30px', borderTop: '1px solid #ddd', paddingTop: '20px', display: 'flex', gap: '15px' }}>
                                    <button onClick={deactivateAccount} style={{ ...buttonStyle('#e67e22'), width: 'auto', fontSize: '0.9em', padding: '10px 20px' }}>⏸️ Deactivate</button>
                                    <button onClick={deleteAccount} style={{ ...buttonStyle('#c0392b'), width: 'auto', fontSize: '0.9em', padding: '10px 20px' }}>⚠️ Delete Account</button>
                                </div>
                            ...
                            <div style={{ marginTop: '10px' }}>
                                <h4>✏️ Edit Account Profile</h4>
                                <label style={{ fontSize: '0.9em', color: '#666' }}>Name</label>
                                <input placeholder="Name" value={editName} onChange={e => setEditName(e.target.value)} style={{ ...inputStyle, marginBottom: '10px' }} />
                                <label style={{ fontSize: '0.9em', color: '#666' }}>Phone</label>
                                <input placeholder="Phone" value={editPhone} onChange={e => setEditPhone(e.target.value)} style={{ ...inputStyle, marginBottom: '10px' }} />
                                <label style={{ fontSize: '0.9em', color: '#666' }}>Address</label>
                                <input placeholder="Address" value={editAddress} onChange={e => setEditAddress(e.target.value)} style={{ ...inputStyle, marginBottom: '15px' }} />

                                <div style={{ marginTop: '15px', padding: '10px', background: '#f0f0f0', borderRadius: '5px' }}>
                                    <h5 style={{ margin: '0 0 10px 0' }}>🔔 Notifications</h5>
                                    <label style={{ display: 'block', marginBottom: '5px' }}><input type="checkbox" checked={editNotifEmail} onChange={e => setEditNotifEmail(e.target.checked)} /> Email Alerts</label>
                                    <label style={{ display: 'block', marginBottom: '5px' }}><input type="checkbox" checked={editNotifSMS} onChange={e => setEditNotifSMS(e.target.checked)} /> SMS Alerts</label>
                                </div>
            ...
            {
                (view === 'feed' || view === 'favorites' || view === 'history') && view !== 'donate' && view !== 'admin' && view !== 'profile' && (
                    <div className="card" style={currentCardStyle}>
                        <div className="card" style={{ ...currentCardStyle, display: 'flex', gap: '10px', padding: '15px', flexWrap: 'wrap' }}>
                            ...
                            <button onClick={() => { setView('history'); fetchMyHistory(); }} style={{ ...tabButtonStyle, background: view === 'history' ? '#2c3e50' : '#f9f9f9', color: view === 'history' ? 'white' : 'black' }}>{t.history}</button>
                            {user.id && <button onClick={() => setView('profile')} style={{ ...tabButtonStyle, background: view === 'profile' ? '#2c3e50' : '#f9f9f9', color: view === 'profile' ? 'white' : 'black' }}>👤 Profile</button>}
                        </div>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
                        ...
                        {view === 'feed' && (
                            <div style={{ display: 'flex', gap: '12px', marginBottom: '25px', background: highContrast ? '#222' : '#f8f9fa', padding: '15px', borderRadius: '8px', flexWrap: 'wrap', border: highContrast ? '1px solid #ffd700' : 'none' }}>
                                <div style={{ flex: 2 }}>
                                    <input placeholder={t.search} value={searchTerm} onChange={e => {
                                ...
                                <div key={item._id || item.id} style={{
                                    ...
                                    <div style={{ display: 'flex', gap: '15px', alignItems: 'flex-start' }}>
                                        ...
                                        <div style={{ flex: 1 }}>
                                                ...
                                                <div style={{ marginTop: '10px', display: 'flex', flexDirection: 'column', gap: '10px' }}>
                                                    {!item.isReadyForPickup ? (
                                                        <div style={{ background: '#fff3cd', padding: '10px', borderRadius: '5px', fontSize: '0.9em' }}>
                                                            ⏳ Waiting for Donor to mark as "Ready for Pickup"
                                                        </div>
                                                    ) : (
                                                        <div style={{ background: '#e3f2fd', padding: '10px', borderRadius: '8px', border: '1px solid #bbdefb' }}>
                                                            <p style={{ fontWeight: 'bold', margin: '0 0 5px 0', fontSize: '0.9em' }}>Ready for Pickup!</p>
                                                            <p style={{ fontSize: '0.8em', margin: '0 0 10px 0' }}>1. Ask Donor for QR Code<br />2. Take a clear photo of it:</p>
                                                            <input type="file" accept="image/*" onChange={handleImageUpload} style={{ fontSize: '0.8em' }} />

                                                            <div style={{ marginTop: '10px', display: 'flex', gap: '10px' }}>
                                                                {user.isTrained ? (
                                                                    <button
                                                                        onClick={() => updateStatus(item._id || item.id, 'In Transit', deliveryProof)}
                                                                        style={{ ...actionButtonStyle('#8e44ad'), opacity: deliveryProof ? 1 : 0.5 }}
                                                                ...
                                                                <button onClick={reportArrived} style={actionButtonStyle('#34495e')}>I've Arrived</button>
                                                            </div>
                                                        </div>
                                                    )}
                                                    <button onClick={() => updateStatus(item._id || item.id, 'Cancelled')} style={{ ...actionButtonStyle('#c0392b'), alignSelf: 'flex-start' }}>⚠️ Issue / Missing</button>
                                                </div>
                                            )}
                                            ...
                                            {user.role === 'Admin' && isExpired(item.createdAt, item.expiry_hours) && (
                                                <button onClick={() => deleteListing(item._id || item.id)} style={actionButtonStyle('#c0392b')}>
                                                    🗑️ Delete Expired
                                                </button>
                                            )}
                                            ...
                                            {item.status === 'Claimed' && user.role === 'Donor' && isSameUser(item.donor, user.id) && (
                                                <div style={{ marginTop: '10px' }}>
                                                    {!item.isReadyForPickup ? (
                                                        <button onClick={() => updateStatus(item._id || item.id, 'ReadyToPickup')} style={actionButtonStyle('#27ae60')}>
                                                            📦 Mark Ready for Pickup
                                                        </button>
                                                    ) : (
                                                        <div style={{ background: 'white', padding: '15px', borderRadius: '10px', border: '2px solid #27ae60', textAlign: 'center' }}>
                                                            <p style={{ fontWeight: 'bold', color: '#27ae60', margin: '0 0 10px 0' }}>✨ Ready for Pickup</p>
                                                            <QRCode value={`FOOD_ID:${item._id}`} size={140} />
                                                            <p style={{ fontSize: '0.8em', color: '#777', marginTop: '10px' }}>Show this QR to the Volunteer</p>
                                                        </div>
                                                    )}
                                                </div>
                                            )}
                                            ...
                                            {item.status === 'In Transit' && user.role === 'NGO' && isSameUser(item.claimedBy, user.id) && (
                                                <div style={{ marginTop: '10px', background: '#e8f5e9', padding: '15px', borderRadius: '8px', border: '1px solid #c8e6c9' }}>
                                                    <p style={{ fontWeight: 'bold', margin: '0 0 10px 0', color: '#2e7d32' }}>🚚 Out for Delivery</p>
                                                    <button
                                                        onClick={() => {
                                                            setScanTargetId(item._id || item.id);
                                                            setIsScanning(true);
                                                        }}
                                            ...
                                            <div style={{ display: 'flex', gap: '10px', marginTop: '15px' }}>
                                                {isSameUser(item.donor, user.id) && (
                                                    item.status === 'Available' ? (
                                                        ...
                                                        isExpired(item.createdAt, item.expiry_hours) && item.status !== 'Delivered' && (
                                                            <button onClick={() => deleteListing(item._id || item.id)} style={{ ...buttonStyle('#e74c3c'), width: 'auto', padding: '8px 15px' }}>🗑️ Delete Expired</button>
                                                        )
                                                    )
                                                )}
            ...
            {isScanning && (
                <div style={{
                    ...
                    <div style={{ background: 'white', padding: '20px', borderRadius: '15px', width: '90%', maxWidth: '400px', textAlign: 'center' }}>
                        <h3 style={{ margin: '0 0 15px 0', color: '#333' }}>📷 Scan Donor QR</h3>
                        <div id="reader" style={{ width: '100%' }}></div>
                        <p style={{ fontSize: '0.9em', color: '#666', marginTop: '15px' }}>
                            Align the QR code within the frame to verify delivery.
                        </p>
                        <button
                            onClick={() => { setIsScanning(false); setScanTargetId(null); }}
                            style={{ ...buttonStyle('#e74c3c'), marginTop: '20px' }}
                        >
                            Cancel Scan
                        </button>
                    </div>
